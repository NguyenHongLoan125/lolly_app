...
class PostedDishItemWidget extends StatefulWidget {
  final Map<String, dynamic> dishData;
  const PostedDishItemWidget({super.key, required this.dishData});

  @override
  State<PostedDishItemWidget> createState() => _PostedDishItemWidgetState();
}

class _PostedDishItemWidgetState extends State<PostedDishItemWidget> {
  bool isDelete = false;
  ...
  Widget build(BuildContext context) {
    if (isDelete) return const SizedBox.shrink();
    return Padding(
                      ...
                      children: [
                        ...
                        TextButton.icon(
                          onPressed: () async {
                            ...
                            await _controller.deteteDish(dishId);

                            setState(() => isDelete = true);

                            ScaffoldMessenger.of(context).showSnackBar(
                              const SnackBar(content: Text("Đa xóa món ăn.")),
                            );
                          },

                          icon: const Icon(Icons.delete, color: Color(0xFF007400)),
                          label: const Text(
                            "Xóa",
